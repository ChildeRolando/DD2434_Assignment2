...
class Tree:
    ...
    def calculate_prob_distribution(self):
        ...
        while len(visit_list):
            ...
            visit_list = visit_list[1:]
            visit_list = visit_list + cur_node.descendants
            if not cur_node.ancestor:
                cur_node.prob = cur_node.cat
                continue
            else:
                p0 = cur_node.ancestor.prob
                A = [i.tolist() for i in cur_node.cat]
            cur_node.prob = np.dot(p0, A).tolist()

    def calculate_likelihood(self, beta):
        ...
        while len(visit_list):
            ...
            visit_list = visit_list[1:]
            visit_list = visit_list + cur_node.descendants
            if not len(cur_node.descendants):
                prob *= cur_node.prob[int(beta_list[int(cur_node.name)])]
        return prob
...
def main():
    ...
    t = Tree()
    t.print()
    with open('data\q2_2\q2_2_large_tree.pkl', 'rb') as in_file:
        t_temp = pickle.load(in_file)
        t_temp.calculate_prob_distribution()
